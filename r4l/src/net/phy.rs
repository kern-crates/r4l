...
/// PhyDevice Trait
///
/// Each OS should implement this trait
pub(crate) trait PhyDevice {
    fn pyh_id(&self) -> u32;
    fn state(&self) -> DeviceState;
    fn is_link_up(&self) -> bool;
    fn is_autoneg_completed(&self) -> bool;
    fn set_speed(&mut self) -> bool;
...
#[repr(transparent)]
pub struct Device<T:PhyDevice>(T);

impl <T:PhyDevice> Device {
    /// New instance Device
    ///
    /// On linux this API is Create a new [`Device`] instance from a raw pointer.
    /// Because there is no external pointer like Linux on Rust OS, the from_raw interface is no
    /// longer needed, and it is replaced by new
    fn new(inner: T) -> Self {
        Device(inner)
    }
